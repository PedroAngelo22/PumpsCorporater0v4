...
if st.session_state.get("authentication_status"):
    ...
    try:
        ...
        # *** INÍCIO DO BLOCO DE CÁLCULO CORRIGIDO ***
        
        # Calcular carga de pressão manométrica na sucção (se houver)
        ...
        if vazao_op is not None and altura_op is not None:
            ...
            ax_curvas.plot(vazao_range, altura_bomba, label='Curva da Bomba', color='royalblue', lw=2); ax_curvas.plot(vazao_range, altura_sistema, label='Curva do Sistema', color='seagreen', lw=2); ax_curvas.scatter(vazao_op, altura_op, color='red', s=100, zorder=5, label=label_ponto_op)
            ...
            st.divider()
            st.header("📈 Gráfico de Curvas: Bomba vs. Sistema"); st.pyplot(fig_curvas); plt.close(fig_curvas)
            st.divider()
